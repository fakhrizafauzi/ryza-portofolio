...
export function LayoutForm() {
    ...
    return (
        <div className="space-y-8">
            ...
            {showTemplatePicker && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 border-2 border-primary/20 p-6 rounded-3xl bg-primary/5">
                    <div className="col-span-full flex justify-between items-center mb-2">
                        <h4 className="font-bold flex items-center gap-2 text-primary">
                            <Layout className="w-5 h-5" /> Select a Template
                        </h4>
                        <Button variant="ghost" size="sm" onClick={() => setShowTemplatePicker(false)}>Cancel</Button>
                    </div>
                    {SECTION_TEMPLATES.map(t => (
                        <button
                            key={t.id}
                            onClick={() => handleAddSection(t.id)}
                            className="p-4 rounded-2xl bg-card border hover:border-primary hover:shadow-md transition-all text-left space-y-2 group"
                        >
                            <div className="flex justify-between items-center">
                                <span className="font-bold group-hover:text-primary">{t.name}</span>
                                <ChevronRight className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity" />
                            </div>
                            <p className="text-xs text-muted-foreground">{t.description}</p>
                            <span className="text-[10px] uppercase tracking-widest font-bold text-primary/60">{t.type}</span>
                        </button>
                    ))}
                </div>
            )}
            ...
            <div className="space-y-4">
                {loading ? (
                    ...
                    sections.map((section, index) => (
                        <div key={section.id} className="group relative bg-card/50 hover:bg-card border rounded-3xl p-6 transition-all shadow-sm hover:shadow-md">
                            {editingSection?.id === section.id ? (
                                <form onSubmit={handleUpdate} className="space-y-6">
                                    ...
                                    <div className="flex justify-end gap-2">
                                        <Button type="button" variant="ghost" onClick={() => setEditingSection(null)}>Cancel</Button>
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-6">
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleMove(index, 'down')} disabled={index === sections.length - 1}>
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button variant="ghost" size="icon" className="text-destructive" onClick={() => setConfirmDelete({ id: section.id, title: section.title })}>
